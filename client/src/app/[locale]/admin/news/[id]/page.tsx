...
export default function EditNewsPage() {
  ...
  useEffect(() => {
    api.getAdminNewsDetail(id).then((n) => {
      setForm({
        titleRu: n.titleRu, titleTj: n.titleTj, titleEn: n.titleEn,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">Редактировать новость #{id}</h1>
          <p className="text-sm text-gray-500 mt-0.5">Обновите контент на нужных языках</p>
        </div>
      </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              ...
              {form.imageUrl ? (
                ...
                <label className="flex flex-col items-center justify-center h-40 border-2 border-dashed border-gray-200 rounded-lg cursor-pointer hover:border-[var(--color-primary)] hover:bg-[var(--color-primary)]/5 transition-colors">
                  ...
                  <span className="text-xs text-gray-400">{uploading ? "Загрузка..." : "Нажмите для загрузки"}</span>
